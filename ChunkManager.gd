...
func _init(inputSeed : int, playerCharacter, renderDistance : int, chunk_Size : int, tile_size : float):
	seed = inputSeed
...
func spawn_chunk(cx: int, cz: int):
	...
	chunk.chunk_x = cx
	chunk.chunk_z = cz
	chunk.seed = get_chunk_seed(cx, cz)
...
func get_chunk_seed(cx: int, cz: int) -> int:
	var hashedSeed = hash(str(seed, "_", cx, "_", cz))
	return hashedSeed
